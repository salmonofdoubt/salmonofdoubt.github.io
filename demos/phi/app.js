...
/* === EcoLogits summary helper start === */
function buildEcoLogitsSummary() {
  const avoidedCalls = 1;
  const lowWh = 0.18 * avoidedCalls;
  const centralWh = 0.34 * avoidedCalls;
  const highWh = 0.67 * avoidedCalls;
  const centralKwh = centralWh / 1000;

  return `## EcoLogits summary

This memo was generated by a static browser-based demonstrator.

Measured or directly knowable from the design:
- The demo runs locally in the browser.
- It uses local storage only.
- It does not require login, a database, server-side processing, or an AI API call.
- It does not send the decision memo to an external model.

Estimated AI use avoided:
- Avoided AI calls counted: ${avoidedCalls} decision-memo generation equivalent.
- Estimated avoided AI inference electricity: about ${centralWh.toFixed(2)} Wh.
- Plausible avoided range: ${lowWh.toFixed(2)}-${highWh.toFixed(2)} Wh.
- In kWh: about ${centralKwh.toFixed(5)} kWh for this memo-generation step.

Interpretation:
This is an avoided-inference estimate, not a measured footprint. It compares this browser-generated memo with a scenario where the same memo would have been generated through one external AI text-generation call. It is intentionally shown as a small per-use estimate because false precision would be misleading.

Not measured here:
- Exact electricity use from the user's device.
- Exact energy, water, or carbon impact from page hosting and data transfer.
- Any footprint from separate AI prompts the user may choose to run outside this demo.
- Water or carbon savings, because those require provider-specific infrastructure, location, cooling, and grid-carbon assumptions.

Practical interpretation:
The direct footprint of this demo is expected to be small compared with AI-heavy tools because the decision workflow is computed in the browser. Any additional AI footprint should be attributed separately to the external AI service used, the number and length of prompts, and the model/provider infrastructure.

EcoLogits principle:
Report measured data where available, use transparent estimates where necessary, and avoid false precision.`;
}
/* === EcoLogits summary helper end === */
