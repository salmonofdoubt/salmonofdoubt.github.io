...
const els={
  ...
  statMedianAge:$("statMedianAge"),statStations:$("statStations"),historyChart:$("historyChart"),chartMidYear:$("chartMidYear"),qualityChart:$("qualityChart"),qualityYear:$("qualityYear"),qualitySummary:$("qualitySummary"),share:$("shareButton"),zoomIn:$("mapZoomIn"),zoomOut:$("mapZoomOut"),zoomReset:$("mapZoomReset"),zoomLabel:$("mapZoomLabel")
};
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260920-27.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v27")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260920-27.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
