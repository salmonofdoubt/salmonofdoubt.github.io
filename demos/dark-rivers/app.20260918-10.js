...
const els={
  ...
  statMedianAge:$("statMedianAge"),statStations:$("statStations"),historyChart:$("historyChart"),chartMidYear:$("chartMidYear")
};
...
const STATUS_STROKES=Object.freeze({High:"#63a8be",Good:"#76a98b",Moderate:"#b9a25d",Poor:"#b97a57",Bad:"#a95d69"});
...
const UNOBSERVED_STROKE="#263941";
...
function setVisual(path,obs,selected=false){
  ...
  const opacity=selected?1:(obs?(AGE_OPACITY[age]||.76):.58);
  const width=selected?3.15:(obs?1.95:1.05);
...
function select(id){state.selected=id;render(false)}
...
function configureData(){
  ...
  if(official)renderOfficialGeometry();
  else hydrateStaticFallback();
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260918-10.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v10")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260918-10.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
