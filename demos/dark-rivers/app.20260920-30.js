...
const els={
  map:$("riverMap"),touchPan:$("mapTouchPan"),shell:document.querySelector(".map-shell"),coast:$("irelandCoastline"),world:$("mapWorld"),island:document.querySelector(".island"),network:$("riverNetwork"),base:$("riverBase"),continuity:$("riverContinuity"),systems:$("riverSystems"),flashes:$("flashLayer"),
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260920-30.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v30")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260920-30.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
