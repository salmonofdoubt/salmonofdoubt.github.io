...
function setVisual(path,obs,selected=false){
  ...
  const opacity=selected?1:(obs?(AGE_OPACITY[age]||.76):.44);
  const width=selected?2.55:(obs?1.35:.72);
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260918-11.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v11")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260918-11.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
