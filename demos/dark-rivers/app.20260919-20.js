...
function configureData(){
  ...
  if(els.modeNote){
    els.modeNote.innerHTML=official
      ?'<strong>EPA data:</strong> bright Q-value colour marks a sampled section; softly lit neighbouring threads show mapped connectivity, not observed ecological quality.'
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260919-20.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v20")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260919-20.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
