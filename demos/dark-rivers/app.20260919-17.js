...
const els={
  map:$("riverMap"),island:document.querySelector(".island"),network:$("riverNetwork"),base:$("riverBase"),systems:$("riverSystems"),flashes:$("flashLayer"),
...
function applyHydroTransform(){
  ...
  for(const layer of [els.base,els.systems,els.network,els.flashes])layer.setAttribute("transform",transform);
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260919-17.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v17")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260919-17.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
