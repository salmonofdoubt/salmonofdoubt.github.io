...
function fitOfficialGeometry(){
  ...
  const sourceRatio=networkBox.width/networkBox.height;
  const xCorrection=Math.max(.50,Math.min(1,.72/sourceRatio));
  ...
  const target={x:62,y:42,width:876,height:mapViewportHeight-84};
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260919-22.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v22")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260919-22.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
