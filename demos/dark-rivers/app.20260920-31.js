...
const els={
  map:$("riverMap"),touchPan:$("mapTouchPan"),shell:document.querySelector(".map-shell"),coast:$("irelandCoastline"),world:$("mapWorld"),island:document.querySelector(".island"),network:$("riverNetwork"),base:$("riverBase"),mobileRaster:$("mobileRiverRaster"),continuity:$("riverContinuity"),systems:$("riverSystems"),flashes:$("flashLayer"),
...
const state={year:1971,playing:false,selected:null,last:0,carry:0,raf:null,prompt:null,flashTimers:[],mapZoom:1.05,mapPanX:-25,mapPanY:-18,mapDragging:false,mapDragStart:null,mapTouchPanEnabled:false,mapPanFrame:null,mapPanPending:null};
...
function advanceBeat(elapsedBeats=1){
  ...
  // Limit overlapping flash timers on low-power mobile hardware.
  scheduleFlashes(batch,matchMedia("(max-width:650px)").matches?420:690);
}
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260920-31.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v31")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260920-31.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
