...
function choosePulseEvents(events,maxVisual=34){
...
function scheduleFlashes(events,windowMs=1050){
  if(!events.length)return;
  clearFlashTimers();

  const sampled=choosePulseEvents(events,34);
...
function showFlash(e){
  ...
  if(reduce){
    setTimeout(()=>{pulsePath?.remove();core.remove();ring.remove()},320);
    return;
  }
  ...
  if(core.animate){
    const a=core.animate([
      ...
      {opacity:0,r:1.05}
    ],{duration:720,easing:"cubic-bezier(.18,.72,.22,1)"});
  ...
  if(ring.animate){
    ...
    ],{duration:980,easing:"cubic-bezier(.12,.58,.22,1)"});
...
function setYear(y,flash=false){
  ...
  render(false);
  if(flash)scheduleFlashes(data.events.filter(e=>e.year===state.year),920);
}
...
function advanceBeat(){
  ...
  setYear(to,false);
  scheduleFlashes(eventsBetween(from,to),1080);
}
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260918-15.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v15")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260918-15.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
