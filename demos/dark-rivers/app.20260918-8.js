(()=>{"use strict";
const BUILD="20260918-8",NS="http://www.w3.org/2000/svg",fallbackData=window.DARK_RIVERS_DATA,config=window.DARK_RIVERS_CONFIG||{doi:"10.5281/zenodo.0000000",doiUrl:"https://zenodo.org/records/0000000"};
...
function validOfficial(payload){return payload&&payload.meta?.official===true&&Array.isArray(payload.events)&&payload.events.length>0&&Array.isArray(payload.network)&&payload.network.length>0&&Array.isArray(payload.reaches)}
async function loadData(){try{const response=await fetch("./data/official.json",{cache:"no-store"});if(response.ok){const payload=await response.json();if(validOfficial(payload))return payload}}catch(error){console.info("Dark Rivers: using bundled fallback data.",error)}return fallbackData}
...
function addBaseNetwork(){
  ...
  if(!data.meta?.official||!Array.isArray(data.network))return;
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260918-8.js",location.href).href;
    ...
    if("caches" in window){for(const key of await caches.keys()){if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v8")await caches.delete(key)}}
    const reg=await navigator.serviceWorker.register("./service-worker.20260918-8.js",{scope:"./"});
    reg.update().catch(()=>{});
