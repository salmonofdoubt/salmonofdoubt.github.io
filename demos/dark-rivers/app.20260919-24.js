...
function fitOfficialGeometry(){
  let networkBox;
  try{networkBox=els.network.getBBox()}catch{return}
  if(!networkBox||!networkBox.width||!networkBox.height)return;

  // The EPA river centreline and the Natural Earth coastline are both stored
  // in the same longitude/latitude-derived source coordinates (project()).
  // Use one common world transform and fit BOTH layers, not just the rivers.
  // Source SVG x/y are separately scaled longitude/latitude degrees.
  // Apply a fixed geographic equirectangular correction at Ireland’s mid-latitude.
  // Never derive the shape of the island from the extent of sampled EPA reaches.
  ...
  let west=networkBox.x,east=networkBox.x+networkBox.width;
  let north=networkBox.y,south=networkBox.y+networkBox.height;
  try{
    const coast=els.coast.getBBox();
    if(coast.width>0&&coast.height>0){
      west=Math.min(west,coast.x);
      east=Math.max(east,coast.x+coast.width);
      north=Math.min(north,coast.y);
      south=Math.max(south,coast.y+coast.height);
    }
  }catch{}
  const target={x:98,y:54,width:804,height:mapViewportHeight-108};
  const scale=Math.min(target.width/((east-west)*xCorrection),target.height/(south-north));
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260919-24.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v24")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260919-24.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
