...
const els={
  map:$("riverMap"),touchPan:$("mapTouchPan"),shell:document.querySelector(".map-shell"),coast:$("irelandCoastline"),world:$("mapWorld"),island:document.querySelector(".island"),network:$("riverNetwork"),base:$("riverBase"),mobileRaster:$("mobileRiverRaster"),continuity:$("riverContinuity"),systems:$("riverSystems"),
  ...
  stateBar:$("stateBar"),stateLegend:$("stateLegend"),mobileCoverage:$("mobileCoverage"),statYear:$("statYear"),statBeat:$("statBeat"),statRevealed:$("statRevealed"),
...
let rasterNetworkActive=false,rasterNetworkFailed=false;
// Exact projected polyline length computed from the same official networkPath
// used to generate the mobile PNG. SVG getTotalLength is unnecessary on mobile.
const MOBILE_NETWORK_SVG_LENGTH=124271.55;
let qualityTimeline=[];
...
const state={year:1971,playing:false,selected:null,last:0,carry:0,raf:null,prompt:null,mapZoom:1.05,mapPanX:-25,mapPanY:-18,mapDragging:false,mapDragStart:null,mapTouchPanEnabled:false,mapPanFrame:null,mapPanPending:null};
...
// Index the cumulative observation state once. Mobile playback must not rescan
// all historical records for every year or reconstruct 5,000 reach states afresh.
let latestByYear=new Map();
function buildObservationIndex(){
  latestByYear=new Map();
  const grouped=new Map();
  for(const e of data.events){
    const year=Number(e.year);
    if(!e.reachId||!Number.isFinite(year))continue;
    if(!grouped.has(year))grouped.set(year,[]);
    grouped.get(year).push(e);
  }
  const latest=new Map();
  for(let y=Number(data.meta.minYear);y<=Number(data.meta.maxYear);y++){
    for(const e of grouped.get(y)||[])latest.set(e.reachId,e);
    latestByYear.set(y,new Map(latest));
  }
}
const latestAt=year=>latestByYear.get(Number(year))||new Map();
...
function addBaseNetwork(){
  ...
  // Mobile draws the exact same 93,905 mapped river segments from a cached
  // PNG; never parse and paint a 4.5-million-character SVG path twice there.
  if(rasterNetworkActive)return;
  let d="";
...
function measureReachWeights(){
  ...
  fullNetworkWeight=rasterNetworkActive?MOBILE_NETWORK_SVG_LENGTH:0;
  if(!rasterNetworkActive){
    try{
...
function applyHydroTransform(updateLabels=true){
  ...
  // Sea-label measurements force layout; postpone them while dragging.
  if(updateLabels)updateSeaLabels();
}
...
function setupMapNavigation(){
  ...
  // A touch swipe scrolls the document by default. Map movement on touch
  // requires the visible Move map button, so scrolling past the map is not trapped.
  els.touchPan?.addEventListener("click",()=>{
    state.mapTouchPanEnabled=!state.mapTouchPanEnabled;
    els.touchPan.setAttribute("aria-pressed",String(state.mapTouchPanEnabled));
    els.touchPan.textContent=state.mapTouchPanEnabled?"Done moving":"Move map";
    els.map.classList.toggle("is-touch-panning",state.mapTouchPanEnabled);
  });

  // Two-finger gestures zoom the *map*, not the entire page. One-finger
  // gestures retain the browser's native vertical page scrolling.
  let pinch=null,pinchFrame=null,pinchPending=null;
  ...
  function pinchTransform(start,center,distance,base){
    const zoom=Math.max(MIN_MAP_ZOOM,Math.min(MAX_MAP_ZOOM,start.zoom*distance/start.distance));
    return {
      zoom,
      panX:center.x-(base.a*start.sourceX+base.e)*zoom,
      panY:center.y-(base.d*start.sourceY+base.f)*zoom
    };
  }
  function applyPinch(next){
    state.mapZoom=next.zoom;
    state.mapPanX=next.panX;
    state.mapPanY=next.panY;
    applyHydroTransform(false);
  }
  function finishPinch(){
    if(!pinch)return;
    if(pinchFrame!==null){cancelAnimationFrame(pinchFrame);pinchFrame=null;}
    if(pinchPending)applyPinch(pinchPending);
    pinchPending=null;
    pinch=null;
    updateSeaLabels();
  }
  function beginPinch(touches){
    ...
    const matrix=els.map.getScreenCTM();
    if(!matrix)return;
    // Cancel a one-finger map drag if the second finger arrives.
    if(state.mapDragging){
      cancelPanFrame();
      const id=state.mapDragStart?.pointerId;
      state.mapDragging=false;
      state.mapDragStart=null;
      if(id!==undefined&&els.map.hasPointerCapture?.(id))els.map.releasePointerCapture(id);
      els.map.classList.remove("is-panning");
    }
    const inverse=matrix.inverse();
    const midpoint=pinchMidpoint(touches);
    const point=els.map.createSVGPoint();
    point.x=midpoint.x;point.y=midpoint.y;
    const anchor=point.matrixTransform(inverse);
    const zoom=state.mapZoom;
    pinch={
      distance,zoom,inverse,
      sourceX:(anchor.x-hydroBase.e*zoom-state.mapPanX)/(hydroBase.a*zoom),
      sourceY:(anchor.y-hydroBase.f*zoom-state.mapPanY)/(hydroBase.d*zoom)
    };
    els.shell?.classList.add("has-pinched");
  }
  els.map.addEventListener("touchstart",event=>{
  ...
  els.map.addEventListener("touchmove",event=>{
    if(event.touches.length!==2){
      if(pinch)finishPinch();
      return; // single-finger page scrolling must remain native
    }
    if(event.cancelable)event.preventDefault();
    if(!pinch)beginPinch(event.touches);
    if(!pinch)return;
    const midpoint=pinchMidpoint(event.touches);
    const point=els.map.createSVGPoint();
    point.x=midpoint.x;point.y=midpoint.y;
    const center=point.matrixTransform(pinch.inverse);
    pinchPending=pinchTransform(pinch,center,pinchDistance(event.touches),hydroBase);
    if(pinchFrame!==null)return;
    pinchFrame=requestAnimationFrame(()=>{
      pinchFrame=null;
      if(!pinch||!pinchPending)return;
  ...
  els.map.addEventListener("touchend",stopPinch);
  els.map.addEventListener("touchcancel",stopPinch);

  function cancelPanFrame(){
    if(state.mapPanFrame!==null){cancelAnimationFrame(state.mapPanFrame);state.mapPanFrame=null;}
    state.mapPanPending=null;
  }
  function updatePanAt(point){
    if(!state.mapDragStart)return;
    const nextX=state.mapDragStart.panX+point.x-state.mapDragStart.x;
    const nextY=state.mapDragStart.panY+point.y-state.mapDragStart.y;
    state.mapPanPending={x:nextX,y:nextY};
    if(state.mapPanFrame!==null)return;
    state.mapPanFrame=requestAnimationFrame(()=>{
      state.mapPanFrame=null;
      const next=state.mapPanPending;
      state.mapPanPending=null;
      if(!next||!state.mapDragging)return;
      state.mapPanX=next.x;
      state.mapPanY=next.y;
      applyHydroTransform(false);
    });
  }
  ...
  els.map.addEventListener("pointerdown",event=>{
    ...
    // Without explicit map mode, never capture a touch or block page swipes.
    if(touch&&(!state.mapTouchPanEnabled||pinch))return;
    if(!touch&&event.pointerType!=="mouse"&&event.pointerType!=="pen")return;
    if(!state.mapTouchPanEnabled&&event.target.closest?.(".river-reach"))return;
    const p=svgPointFromClient(event.clientX,event.clientY);
    state.mapDragging=true;
    state.mapDragStart={x:p.x,y:p.y,panX:state.mapPanX,panY:state.mapPanY,pointerId:event.pointerId};
    els.map.setPointerCapture?.(event.pointerId);
    els.map.classList.add("is-panning");
  });

  els.map.addEventListener("pointermove",event=>{
    if(!state.mapDragging||!state.mapDragStart||event.pointerId!==state.mapDragStart.pointerId)return;
    updatePanAt(svgPointFromClient(event.clientX,event.clientY));
  });

  const stopPan=event=>{
    if(!state.mapDragging||!state.mapDragStart||event.pointerId!==state.mapDragStart.pointerId)return;
    const final=event.type==="pointerup"&&state.mapPanPending?state.mapPanPending:null;
    cancelPanFrame();
    if(final){state.mapPanX=final.x;state.mapPanY=final.y;}
    state.mapDragging=false;
    state.mapDragStart=null;
    if(els.map.hasPointerCapture?.(event.pointerId))els.map.releasePointerCapture(event.pointerId);
    els.map.classList.remove("is-panning");
    // Reposition sea labels once at the end rather than during every pointermove.
    if(final)applyHydroTransform(true);
    else updateSeaLabels();
  };
...
function select(id){state.selected=id;els.detail.hidden=false;render();highlightRiverSystem(id)}
...
function renderAnalytics(latest){
  ...
  if(els.mobileCoverage)els.mobileCoverage.textContent=`${revealedPct.toFixed(1)}% revealed · ${pct.Dark.toFixed(1)}% dark`;
...
// Reuse the REAL charts and network-state card on mobile. No duplicated SVG,
// fake numbers, extra chart renders or separate legends to keep in sync.
function setupResponsiveDashboard(){
  const slot=$("mobileDashboard");
  const analytics=document.querySelector(".analytics-panel");
  const history=els.historyChart?.closest(".chart-card");
  const network=els.stateBar?.closest(".metric-card");
  const live=els.statYear?.closest(".metric-card");
  if(!slot||!analytics||!history||!network||!live)return;
  const breakpoint=matchMedia("(max-width:650px)");
  const arrange=()=>{
    if(breakpoint.matches){
      if(history.parentNode!==slot||network.parentNode!==slot)slot.append(history,network);
      document.documentElement.dataset.darkRiversMobileDashboard="paired";
    }else{
      if(network.parentNode!==analytics)analytics.insertBefore(network,live);
      if(history.parentNode!==analytics)analytics.append(history);
      document.documentElement.dataset.darkRiversMobileDashboard="desktop";
    }
  };
  arrange();
  if(breakpoint.addEventListener)breakpoint.addEventListener("change",arrange);
  else breakpoint.addListener(arrange);
}
...
function advanceBeat(elapsedBeats=1){
  if(state.year>=data.meta.maxYear){setPlaying(false);return}
  const step=Math.max(1,Number(els.speed.value||6))*Math.max(1,elapsedBeats);
...
function tick(now){
  ...
  const beatMs=790;
  const interval=matchMedia("(max-width:650px)").matches?520:beatMs;
  const delta=Math.max(0,now-state.last);
  state.last=now;
  // Never discard time simply because a mobile frame took over 250 ms.
  // Ignore long background-tab gaps so returning to the app does not jump.
  state.carry+=delta>3000?0:delta;
  if(state.carry>=interval){
    const elapsedBeats=Math.min(4,Math.floor(state.carry/interval));
    state.carry%=interval;
    advanceBeat(elapsedBeats);
  }
...
function configureData(){
  ...
  rasterNetworkActive=official&&!rasterNetworkFailed&&matchMedia("(pointer:coarse), (max-width:650px)").matches;
  document.body.classList.toggle("has-official-data",official);
  document.body.classList.toggle("use-raster-network",rasterNetworkActive);
  document.documentElement.dataset.darkRiversRaster=rasterNetworkActive?"on":"off";
  updateMapViewport();
...
function setupInstall(){
  ...
  addEventListener("beforeinstallprompt",e=>{
    e.preventDefault();
    state.prompt=e;
    if(!installed())els.install.hidden=false;
    els.install.classList.add("is-ready");
  });
  addEventListener("appinstalled",()=>{state.prompt=null;els.install.hidden=true;});
  els.install.addEventListener("click",async()=>{
    if(installed()){els.install.hidden=true;return}
    if(state.prompt){
    ...
    const message=ios
      ?'On iPhone or iPad, open this page in <strong>Safari</strong>, tap <strong>Share</strong>, then <strong>Add to Home Screen</strong> and confirm.'
      :android
        ?'On Android, open the browser menu <strong>⋮</strong>, select <strong>Install app</strong> or <strong>Add to Home Screen</strong>, then confirm.'
        :'Use your browser menu to choose <strong>Install app</strong> or <strong>Add to Home Screen</strong>. Some browsers do not offer installation.';
    els.instructions.innerHTML='<p>'+message+'</p>';
...
async function setupPwa(){
  ...
  try{
    const wanted=new URL("./service-worker.20260921-35.js",location.href).href;
    ...
    if("caches" in window){
      for(const key of await caches.keys()){
        if(key.startsWith("salmon-dark-rivers-")&&key!=="salmon-dark-rivers-v35")await caches.delete(key);
      }
    }
    const reg=await navigator.serviceWorker.register("./service-worker.20260921-35.js",{scope:"./"});
    reg.update().catch(()=>{});
  }catch(error){
...
async function init(){
  // If a browser cannot decode the raster, fall back to the complete original
  // vector network rather than silently displaying an empty background.
  els.mobileRaster?.addEventListener("error",()=>{
    rasterNetworkFailed=true;
    if(rasterNetworkActive){
      rasterNetworkActive=false;
      document.body.classList.remove("use-raster-network");
      document.documentElement.dataset.darkRiversRaster="fallback";
      addBaseNetwork();
      measureReachWeights();
    }
  });
  ...
  els.range.addEventListener("input",e=>{setPlaying(false);setYear(e.target.value)});
  els.speed.addEventListener("change",()=>{if(state.playing){state.last=performance.now();state.carry=0}});
  const doiPending=!config.doi||config.doi.includes("0000000");
  els.doi.classList.toggle("is-placeholder",doiPending);
  els.doiText.textContent=doiPending?"Zenodo · DOI pending":config.doi;
  if(doiPending){
    els.doi.removeAttribute("href");
    els.doi.removeAttribute("target");
    els.doi.setAttribute("aria-label","Dark Rivers Zenodo DOI pending publication");
  }else{
    els.doi.href=config.doiUrl;
    els.doi.setAttribute("aria-label","Dark Rivers Zenodo DOI "+config.doi);
  }
  setupInstall();
