(()=>{"use strict";
const NS="http://www.w3.org/2000/svg",fallbackData=window.DARK_RIVERS_DATA,config=window.DARK_RIVERS_CONFIG||{doi:"10.5281/zenodo.0000000",doiUrl:"https://zenodo.org/records/0000000"};
const $=id=>document.getElementById(id),els={map:$("riverMap"),island:document.querySelector(".island"),network:$("riverNetwork"),base:$("riverBase"),flashes:$("flashLayer"),play:$("playButton"),reset:$("resetButton"),range:$("yearRange"),year:$("yearDisplay"),count:$("reachCount"),samples:$("sampleCount"),speed:$("speedSelect"),detail:$("reachDetail"),provenance:$("dataProvenance"),modeNote:$("dataModeNote"),install:$("installApp"),dialog:$("installDialog"),instructions:$("installInstructions"),doi:$("doiPill"),doiText:$("doiText")};
let data=fallbackData;
const state={year:1971,playing:false,selected:null,last:0,carry:0,raf:null,prompt:null},paths=new Map(),reachById=new Map();
...
function configureData(){els.network.replaceChildren();paths.clear();reachById=new Map(data.reaches.map(r=>[r.id,r]));addBaseNetwork();data.reaches.forEach(addReach);const official=data.meta?.official===true;document.body.classList.toggle("has-official-data",official);els.island?.setAttribute("aria-hidden",official?"true":"false");els.range.min=data.meta.minYear;els.range.max=data.meta.maxYear;document.querySelectorAll("[data-min-year]").forEach(el=>el.textContent=data.meta.minYear);document.querySelectorAll("[data-max-year]").forEach(el=>el.textContent=data.meta.maxYear);state.year=Number(data.meta.minYear);if(els.provenance){els.provenance.innerHTML=official?`EPA River Ecology Monitoring Programme · ${data.meta.eventCount?.toLocaleString("en-IE")||data.events.length.toLocaleString("en-IE")} observations · <a href="${escapeHtml(data.meta.sources?.q_map||"https://gis.epa.ie/EPAMaps/Water")}" target="_blank" rel="noopener">source</a>`:"Illustrative interaction data · awaiting generated EPA dataset"}if(els.modeNote){els.modeNote.innerHTML=official?`<strong>EPA data:</strong> each observation is tied to a monitoring station. Colour is shown only on a short mapped section around that station; it does not imply continuous measurement along the river.`:`<strong>Prototype:</strong> river geometry and sampling events shown here are illustrative. Q-value classes follow the EPA system; synthetic events are not presented as EPA observations.`}render(true)}
...
async function init(){data=await loadData();els.play.addEventListener("click",()=>setPlaying(!state.playing));els.reset.addEventListener("click",reset);els.range.addEventListener("input",e=>{setPlaying(false);setYear(e.target.value)});els.doiText.textContent=config.doi;els.doi.href=config.doiUrl;els.doi.classList.toggle("is-placeholder",config.doi.includes("0000000"));setupInstall();setupPwa();configureData()}
init();
})();