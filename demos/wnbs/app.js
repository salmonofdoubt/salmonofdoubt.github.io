...
function wnbsDownloadInsightCard(item) {
...
// --- WNBS CARD PREVIEW HELPERS START ---
function wnbsCardClean(value, fallback = "") {
...
function wnbsBuildInsightCardCanvas(item) {
  const canvas = document.createElement("canvas");
  canvas.width = 1200;
  canvas.height = 1200;
  const ctx = canvas.getContext("2d");
  ...
  function wrap(text, x, y, maxWidth, lineHeight, maxLines) {
    const words = wnbsCardClean(text).split(/\s+/).filter(Boolean);
    const lines = [];
    let line = "";
  ...
  const title = wnbsCardClean(item?.title, "Practical Water NbS signal");
  const source = wnbsCardClean(item?.source_name || item?.source_id, "Source");
  const date = wnbsCardDate(item?.published);
  const insight = wnbsCardInsight(item);
...
function wnbsInsightCardDataUrl(item) {
  return wnbsBuildInsightCardCanvas(item).toDataURL("image/png");
}

function wnbsDownloadInsightCard(item) {
  const canvas = wnbsBuildInsightCardCanvas(item);
  const link = document.createElement("a");
  const safeId = String(item?.id || "water-nbs-signal").replace(/[^a-z0-9_-]+/gi, "-").toLowerCase();
  link.download = `${safeId}.png`;
  link.href = canvas.toDataURL("image/png");
...
function renderCard(item) {
  ...
  node.querySelector(".image-btn").addEventListener("click", () => wnbsDownloadInsightCard(item));

  const previewWrap = document.createElement("div");
  previewWrap.className = "insight-card-preview-wrap";
  ...
  const previewImg = previewWrap.querySelector(".insight-card-preview");
  previewImg.src = wnbsInsightCardDataUrl(item);
  previewImg.alt = `Insight card preview for ${item.title || "story"}`;
  previewImg.addEventListener("click", () => wnbsDownloadInsightCard(item));

  const draftLabel = node.querySelector(".draft-label");
  if (draftLabel) {
    draftLabel.insertAdjacentElement("afterend", previewWrap);
  } else {
